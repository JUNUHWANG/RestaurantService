...
function finddate(restaurant_id){
...
//		location.href = "/reservation/"+restaurant_id+"/finddayrestaurantorders";
		f.submit();
		return true;
	}
}

function findname(restaurant_id){
...
//		location.href = "/reservation/"+restaurant_id+"/findnametoid";
		f.submit();
		return true;
	}
}
	...
	<!-- myorders list -->
		...
		<a name="list-myorders">
			<div class="myorders-info">
				<h1 class="myorders-heading">음식점 ${r.restaurantName} 예약목록</h1>
				<div class="underline">
						...
						<tbody>
							<c:forEach var="r" items="${list }">
								<tr scope="row">
									...
									<td>${r.day }</td><td>${r.time }시</td><td>${r.id.name }</td><td>${r.rev_cnt }</td>
								</tr>
							</c:forEach>
						</tbody>
						</table></div>
