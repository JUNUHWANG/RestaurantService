	...
	<!-- myorders list -->
		...
		<a name="list-myorders">
			<div class="myorders-info">
				<h1 class="myorders-heading">예약자의 id를 선택해주세요</h1>
				<div class="underline">
					<div class="myorders-underline"></div>
					</div>
				<div class="myorders-wrapper">
					<c:if test="${empty list}">
					<h2>해당 이름을 가진 ID가 없습니다.</h2>
					</c:if>
					...
					<div class="myorders-list">
						<table class="table custom-table">
							...
							<tbody>
								...
								<tr scope="row"><td><a href="/reservation/${r_id}/${m.id}/searchnamefindreservation">${m.id}</a></td>
								<td>${m.name }</td><td>${m.tel }</td><td>${m.email }</td>
								</tr>
								</c:forEach>
							</tbody>
							</table></div>
